...
int main() {
    ...
    for (int j = 0; j < NUM_SEMESTERS; ++j) {
        ...
        for (int c = 0; c < NUM_CLASSES_PER_SEMESTER; ++c) {
            ...
            scanf("%f %f %f %f", &modules[j].classes[c].td, &modules[j].classes[c].tp, &modules[j].classes[c].quiz, &modules[j].classes[c].exam);
        }
    }

    for (int j = 0; j < NUM_SEMESTERS; ++j) {
        printf("Results for Semester %d:\n", j + 1);
        displaySemesterResult(&modules[j]);
        checkRemediation(&modules[j]);
    }
